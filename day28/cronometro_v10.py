...
import os  # Para reproduzir o som no Linux

# Variáveis globais
janela_processamento = None  # Janela de processamento
tempo_restante = 0  # Tempo restante do cronômetro

# Função para reproduzir um som no Linux
def reproduzir_som():
    os.system('aplay /usr/share/sounds/sound-icons/capital')  # Reproduz um som do sistema
    # Alternativa: os.system('beep -f 1000 -l 500')  # Requer o pacote 'beep' instalado
...
def finalizar_cronometro():
    ...
    # Reproduzir um som de bip
    reproduzir_som()

    # Exibir mensagem de término
...
def abrir_tela_processamento():
    ...
    # Trazer a janela de processamento para o primeiro plano
    janela_processamento.lift()  # Traz a janela para o topo
    janela_processamento.attributes("-topmost", True)  # Mantém a janela no topo
    janela_processamento.attributes("-topmost", False)  # Permite que outras janelas voltem ao topo
